...
from agentcoop.case_studies.legacy_registry import legacy_case_family
from agentcoop.config import get_benchmark_settings
...
def probe_humaneval_call(
    *,
    ...
    harness = (
        ...
        "try:\n"
        f"    _agentcoop_probe_value = {call_expr}\n"
        "    print(repr(_agentcoop_probe_value))\n"
        "except Exception as exc:\n"
        "    print(type(exc).__name__ + ': ' + str(exc))\n"
        "    raise\n"
    )
    with tempfile.TemporaryDirectory(prefix="agentcoop_humaneval_probe_") as tmp_dir:
...
def prepare_scanpy_pbmc3k_case_study(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    from agentcoop.case_studies.scanpy_pbmc3k_job import DEFAULT_ANALYSIS_CONFIG
...
def prepare_scanpy_paul15_case_study(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    from agentcoop.case_studies.scanpy_paul15_job import DEFAULT_ANALYSIS_CONFIG
...
def prepare_squidpy_visium_case_study(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if interaction_case:
        from agentcoop.case_studies.squidpy_visium_interactions_job import DEFAULT_ANALYSIS_CONFIG
    else:
        from agentcoop.case_studies.squidpy_visium_job import DEFAULT_ANALYSIS_CONFIG
...
def prepare_visium_multi_agent_case_study(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    from agentcoop.case_studies.scanpy_visium_cluster_job import DEFAULT_ANALYSIS_CONFIG as SCANPY_CLUSTER_DEFAULTS
    from agentcoop.case_studies.squidpy_visium_interactions_job import DEFAULT_ANALYSIS_CONFIG as SQUIDPY_INTERACTION_DEFAULTS
...
def _prepare_public_case_data_bundle(
    ...
    *,
    ...
    from agentcoop.case_studies.public_data import (
...
def _execute_humaneval_check(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="agentcoop_humaneval_") as tmp_dir:
...
def _execute_humaneval_assertions(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="agentcoop_humaneval_public_") as tmp_dir:
        script_path = Path(tmp_dir) / "check.py"
        script_path.write_text(harness, encoding="utf-8")
        try:
            completed = subprocess.run(
