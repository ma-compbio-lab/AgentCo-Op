...
class DROPRunner(BenchmarkRunner):
    benchmark_name = "drop"
    default_preferred_nodes = ("selector", "solver")

    def prepare_dataset(self, resolved: Mapping[str, Any]) -> Dict[str, Any]:
        from agentcoop.benchmarks_lib import prepare_drop_dataset
        return prepare_drop_dataset(resolved)

    def build_task_blueprint(
        self, base_blueprint: WorkflowBlueprint, sample: Dict[str, Any]
    ) -> WorkflowBlueprint:
        from agentcoop.experiment_runner import build_drop_task_blueprint
        return build_drop_task_blueprint(base_blueprint, sample)
    ...
    def build_task_result(
        ...
        *,
        ...
        from agentcoop.experiment_runner import _build_drop_task_result
        return _build_drop_task_result(
            sample=sample,
