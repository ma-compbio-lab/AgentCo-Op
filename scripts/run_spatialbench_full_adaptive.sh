...
RUN_TAG="${RUN_TAG:-spatialbench_full_adaptive_$(date +%Y%m%d_%H%M%S)}"
...
OUT_DIR="$OUT_DIR" \
RESULT_PREFIX="adaptive" \
bash scripts/run_spatialbench_adaptive.sh "${RUN_ARGS[@]}" "$@"

echo
echo "Full SpatialBench adaptive done:"
echo "  $OUT_DIR/adaptive_summary.json"
echo "  $OUT_DIR/adaptive_results.jsonl"
