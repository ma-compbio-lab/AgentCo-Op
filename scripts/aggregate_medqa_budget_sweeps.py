...
from dynaforge.experiment_runner import aggregate_medqa_runs


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate MedQA budget sweep shards and emit summary tables.")
    parser.add_argument(
        "--sweeps-root",
        default="runs/medqa-budget-sweeps",
        help="Root directory that contains budget sweep run directories.",
    )
    parser.add_argument(
        "--output-dir",
        default="runs/medqa-budget-sweeps-aggregated",
        help="Directory for aggregated outputs.",
    )
    parser.add_argument(
        "--plot",
        action="store_true",
        help="Generate Pareto plots if matplotlib is available.",
    )
...
def discover_latest_shards(sweeps_root: Path) -> dict[tuple[str, str, str], list[Path]]:
    ...
    for task_results_path in sweeps_root.glob("*/*/*/medqa-full-shard*of*/*/eval/task_results.json"):
        run_dir = task_results_path.parents[2]
...
def pareto_frontier(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        for other in rows:
            ...
            better_or_equal_acc = float(other["accuracy"]) >= float(row["accuracy"])
            strictly_better = (
                float(other["average_usd"]) < float(row["average_usd"])
                or float(other["accuracy"]) > float(row["accuracy"])
            )
            if better_or_equal_cost and better_or_equal_acc and strictly_better:
                dominated = True
                break
        if not dominated:
            frontier.append(row)
    frontier.sort(key=lambda item: (float(item["average_usd"]), -float(item["accuracy"])))
    return frontier
...
def maybe_plot(rows: list[dict[str, Any]], output_dir: Path) -> list[str]:
    ...
    for row in rows:
        ...
        ax.scatter(float(row["average_usd"]), float(row["accuracy"]), label=label)
        ax.annotate(label, (float(row["average_usd"]), float(row["accuracy"])), fontsize=8)
    ax.set_xlabel("Average USD per task")
    ax.set_ylabel("Accuracy")
    ax.set_title("MedQA Pareto: Accuracy vs Cost")
    ax.grid(True, alpha=0.3)
    path = output_dir / "pareto_accuracy_vs_usd.png"
    fig.tight_layout()
    ...
    for row in rows:
        label = f"{row['model']} / {row['budget_tier']}"
        ax.scatter(float(row["average_input_tokens"]), float(row["accuracy"]), label=label)
        ax.annotate(label, (float(row["average_input_tokens"]), float(row["accuracy"])), fontsize=8)
    ax.set_xlabel("Average input tokens per task")
    ax.set_ylabel("Accuracy")
    ax.set_title("MedQA Pareto: Accuracy vs Input Tokens")
    ax.grid(True, alpha=0.3)
    path = output_dir / "pareto_accuracy_vs_input_tokens.png"
    fig.tight_layout()
    fig.savefig(path, dpi=160)
    plt.close(fig)
    paths.append(str(path))
...
def main() -> int:
    ...
    for (experiment, model, tier), run_dirs in sorted(discovered.items()):
        aggregate_summary = aggregate_medqa_runs(
            [str(path) for path in run_dirs],
            base_dir=output_dir / experiment / model / tier,
            run_name="aggregate",
        )
        workflow_patterns = aggregate_summary.get("workflow_patterns", {})
        review_count = int(workflow_patterns.get("planner->responder->reviewer->reviser", 0))
        task_count = int(aggregate_summary.get("task_count", 0) or 0)
        row = {
            "experiment": experiment,
            "model": model,
            "budget_tier": tier,
            "task_count": task_count,
            "accuracy": aggregate_summary.get("accuracy", 0.0),
            ...
            "review_rate": (review_count / task_count) if task_count else 0.0,
    ...
    plot_paths = maybe_plot(rows, output_dir) if args.plot else []

    json_path = output_dir / "budget_sweep_summary.json"
    csv_path = output_dir / "budget_sweep_table.csv"
    md_path = output_dir / "budget_sweep_summary.md"
    ...
    for row in rows:
        lines.append(
            ...
            f"accuracy={row['accuracy']:.4f} average_usd={row['average_usd']:.6f} "
            f"review_rate={row['review_rate']:.4f} average_input_tokens={row['average_input_tokens']:.2f}"
        )
