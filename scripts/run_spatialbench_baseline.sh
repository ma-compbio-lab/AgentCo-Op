...
RUN_TAG="${RUN_TAG:-spatialbench_baseline_$(date +%Y%m%d_%H%M%S)}"
...
CMD=(
  ...
  method=baseline
  # strict single-agent baseline: no tools, no memory, no repair, no planning
  task.allow_web_search_for_spec=off
  task.allow_tool_search=off
  tool.enabled=false
...
echo "Running SpatialBench baseline..."
echo "  env        : $ENV_NAME"
